...
exports.FeatureFlagService = exports.AB_TESTS = exports.FEATURE_FLAGS = void 0;
// ============================================================
// FEATURE FLAG REGISTRY — 40 flags (representative sample)
// ============================================================
const createFlag = (name, description, defaultValue, segments, rollout, owner, extras) => ({
...
class FeatureFlagService {
     ...
     */
    evaluateFlag(flagName, userId, userSegments, environment = 'PRODUCTION') {
        const flag = exports.FEATURE_FLAGS[flagName];
        if (!flag)
            return false;
        ...
        if (flag.killSwitch)
            return false;
        if (flag.scheduledEnable && new Date() < new Date(flag.scheduledEnable))
            return false;
        if (flag.scheduledDisable && new Date() > new Date(flag.scheduledDisable))
            return false;
        // Check prerequisites
        if (flag.prerequisites?.length) {
            for (const prereq of flag.prerequisites) {
                if (!this.evaluateFlag(prereq, userId, userSegments, environment))
                    return false;
            }
        }
        // Check segment match
     ...
     */
    evaluateAllFlags(userId, userSegments, env = 'PRODUCTION') {
        const result = {};
        for (const flagName of Object.keys(exports.FEATURE_FLAGS)) {
     ...
     */
    assignABTestVariant(testName, userId) {
        const test = exports.AB_TESTS[testName];
     ...
     */
    trackABExposure(testName, userId, variantId) {
        return {
     ...
     */
    calculateRequiredSampleSize(baselineRate, mde, alpha = 0.05, power = 0.80) {
