...
exports.DataIntelligenceForensicsService = void 0;
class DataIntelligenceForensicsService {
    /**
     ...
     */
    async detectFraudRings(entities, minRingSize = 3) {
        // Build adjacency map
        const adj = {};
        for (const entity of entities) {
            if (!adj[entity.entityId])
                adj[entity.entityId] = new Set();
            for (const conn of entity.connections) {
                adj[entity.entityId].add(conn.entityId);
                if (!adj[conn.entityId])
                    adj[conn.entityId] = new Set();
     ...
     */
    async analyzeTemporalGraphAnomaly(entityId, connectionHistory) {
...
exports.DataIntelligenceForensicsService = DataIntelligenceForensicsService;
