...
class SemanticSearchService {
     ...
     */
    async searchTransactions(query, topK = 5) {
     ...
     */
    async findNearestNeighbors(queryEmbedding, k = 10) {
     ...
     */
    async detectFraudRings(suspiciousAccountId, maxHops = 3) {
     ...
     */
    async detectDataDrift(referenceDistribution, productionDistribution) {
        // Kolmogorov-Smirnov test for drift detection
        const maxDeviation = Math.max(...referenceDistribution.map((v, i) => Math.abs(v - (productionDistribution[i] ?? 0))));
