...
exports.TemporalFinanceService = void 0;
class TemporalFinanceService {
    /**
     ...
     */
    async detectTemporalArbitrage(assetSymbol, settlementWindowPrices) {
        const opportunities = [];
    ...
    backwardInductionAllocation(params) {
        const monthsToGoal = Math.round((new Date(params.targetDateISO).getTime() - Date.now()) / (30.44 * 86400000));
     ...
     */
    async optimizeSettlementWindow(assetSymbol, transactionValueUSD, availableWindows, currentVolatility // Annualized volatility (e.g. 0.20 = 20%)
    ) {
        ...
        const optimal = windowAnalysis.reduce((best, curr) => curr.totalExpectedCostUSD < best.totalExpectedCostUSD ? curr : best);
        return {
            assetSymbol,
            transactionValueUSD,
            ...
            savingsVsWorstCaseUSD: parseFloat((Math.max(...windowAnalysis.map(w => w.totalExpectedCostUSD)) - optimal.totalExpectedCostUSD).toFixed(2)),
        };
    }
    /**
    ...
    // ─── Private helpers ────────────────────────────────────────────────────────
    getAllocationForReturn(impliedReturn) {
        if (impliedReturn > 0.15)
            return { EQUITY: '90%', ALTERNATIVES: '10%', BONDS: '0%' };
        if (impliedReturn > 0.10)
            return { EQUITY: '75%', ALTERNATIVES: '15%', BONDS: '10%' };
        if (impliedReturn > 0.07)
            return { EQUITY: '60%', BONDS: '30%', ALTERNATIVES: '10%' };
        return { BONDS: '60%', EQUITY: '30%', CASH: '10%' };
    }
    parseWindowDeltaDays(w1, w2) {
        const d1 = parseInt(w1.match(/T\+?(\d+)/)?.[1] ?? '0');
        const d2 = parseInt(w2.match(/T\+?(\d+)/)?.[1] ?? '0');
        return Math.abs(d2 - d1);
    }
}
exports.TemporalFinanceService = TemporalFinanceService;
