...
exports.CreditEngineService = void 0;
...
class CreditEngineService {
     ...
     */
    async calculateInternalCreditScore(userId) {
        ...
        const totalOutflow = transactions.reduce((sum, t) => sum.plus(t.amount.abs()), new decimal_js_1.default(0));
        const anomalyCount = transactions.filter(t => t.anomalyFlag).length;
        let score = 700; // Base score
        if (anomalyCount > 5)
            score -= 100;
        if (totalOutflow.toNumber() > 10000)
            score += 50;
     ...
     */
    async originateLoan(userId, amount, termMonths) {
     ...
     */
    async scheduleRepayments(accountId, loanId, payment) {
...
exports.CreditEngineService = CreditEngineService;
