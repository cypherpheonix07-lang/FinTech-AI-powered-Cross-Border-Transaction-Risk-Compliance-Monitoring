...
export interface ParsedIntent {
  action: 'SEND' | 'CONVERT' | 'AUDIT' | 'UNKNOWN';
  amount?: number;
  currency?: string;
  recipient?: string;
  source?: string;
...
export class IntentEngineService {
  private static instance: IntentEngineService;

  private constructor() {}

  public static getInstance(): IntentEngineService {
   ...
   */
  async parseCommand(input: string): Promise<ParsedIntent> {
    const text = input.toLowerCase();
    
    // Simple heuristic-based parsing for simulation
    let action: ParsedIntent['action'] = 'UNKNOWN';
    if (text.includes('pay') || text.includes('send') || text.includes('transfer')) action = 'SEND';
    if (text.includes('convert') || text.includes('swap')) action = 'CONVERT';
    if (text.includes('audit') || text.includes('check')) action = 'AUDIT';
   ...
   */
  getConfirmationString(intent: ParsedIntent): string {
    if (intent.action === 'UNKNOWN') return "I couldn't quite understand that. Could you try rephrasing?";
...
export const intentEngineService = IntentEngineService.getInstance();
