...
export type PersonalValueSystem = 'SUSTAINABILITY' | 'FAMILY' | 'CREATIVITY' | 'COMMUNITY' | 'ADVENTURE' | 'SECURITY' | 'GROWTH' | 'EXPRESSION';

export interface PersonalCPI {
  userId: string;
  basketItems: { category: string; weight: number; currentPriceIndex: number; baselinePriceIndex: number }[];
  personalInflationRate: number; // Different from CPI headline
  nationalCpiRate: number;
  personalVsNational: number; // How much more/less inflation you face
  calculatedAt: string;
}

export class HyperPersonalizedEconomicsService {
   ...
   */
  async calculatePersonalCPI(
    userId: string,
    spendingData: { category: string; monthlySpendBaseline: number; monthlySpendCurrent: number }[],
    nationalCpiRate: number
  ): Promise<PersonalCPI> {
   ...
   */
  getMicroBudget(monthlyBudgetUSD: number, currentDatetime: Date = new Date()) {
   ...
   */
  async applyValuesFilter(
    userId: string,
    purchase: { merchant: string; category: string; amountUSD: number; merchantEsgScore?: number },
    userValues: PersonalValueSystem[]
  ) {
    const valueConflicts: { value: PersonalValueSystem; concern: string; severity: 'LOW' | 'MEDIUM' | 'HIGH' }[] = [];
   ...
   */
  async detectSpendingHabits(
    monthlyHistory: { date: string; category: string; amountUSD: number; merchantId: string }[]
  ) {
   ...
   */
  async trackSocialCapital(userId: string, activities: {
    type: 'VOLUNTEERING' | 'GIFTING' | 'MENTORING' | 'COMMUNITY_CONTRIBUTION' | 'KNOWLEDGE_SHARING';
    hoursOrUSD: number;
  ...
  async scanBehavioralBiases(decisionContext: {
    consideringPurchase: string;
    triggerEvent?: string; // E.g. "saw ad", "friend mentioned", "market news"
    timeSpentConsideringMinutes: number;
    priorExperienceWithProduct: boolean;
    emotionalStateRating: number; // 1-10: higher = more emotional
    recentLossUSD?: number;
    initialAnchorPriceUSD?: number;
    finalDecisionPriceUSD?: number;
  }) {
    const detectedBiases: { bias: string; evidence: string; debiasStrategy: string }[] = [];
