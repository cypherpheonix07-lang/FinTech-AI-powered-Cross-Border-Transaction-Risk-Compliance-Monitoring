...
export interface NeuralIntention {
  ...
  impulseLevel: number;        // 0-1 (High indicates urgency/weak deliberation)
  cognitiveLoad: number;       // 0-100 (Stress/complexity load)
  congruenceScore: number;     // 0-1 (Alignment with user's core values)
  timestamp: string;
}

export class ThoughtControlService {
  private static instance: ThoughtControlService;

  private constructor() {}

  public static getInstance(): ThoughtControlService {
   ...
   */
  async decodeIntention(userId: string, amount: number): Promise<NeuralIntention> {
   ...
   */
  evaluateIntention(intent: NeuralIntention): { action: 'PROCEED' | 'COOLDOWN' | 'BLOCK'; logic: string } {
   ...
   */
  detectSubliminalPriming(userId: string): { threatDetected: boolean; confidence: number } {
    return {
...
export const thoughtControlService = ThoughtControlService.getInstance();
