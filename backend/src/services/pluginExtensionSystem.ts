...
export type PluginCategory =
  | 'MACHINE_LEARNING' | 'SECURITY' | 'COMPLIANCE' | 'ANALYTICS' | 'NOTIFICATION'
  | 'INTEGRATION' | 'STORAGE' | 'PAYMENT' | 'DOCUMENT' | 'COMMUNICATION'
  | 'UTILITY' | 'MONITORING' | 'BUSINESS' | 'AUTOMATION';

export type PluginStatus = 'ACTIVE' | 'DISABLED' | 'ERROR' | 'DEPRECATED' | 'BETA';
export type PluginTier   = 'FREE' | 'PREMIUM' | 'ENTERPRISE' | 'INTERNAL';
...
export interface PluginManifest {
  ...
  rateLimits?: { callsPerMinute: number; callsPerDay: number };
...
export type PluginHookHandler = (context: PluginContext, payload: unknown) => Promise<PluginResult>;
...
export class TransactionCategorizerPlugin {
  ...
  async onTransactionCreated(ctx: PluginContext, transaction: {
    description: string; amount: number; merchantName?: string; merchantMCC?: string;
...
export class FraudDetectorPlugin {
  ...
  async onTransactionCreated(ctx: PluginContext, payload: {
    transaction: { amount: number; currency: string; merchantName?: string; location?: { lat: number; lng: number }; deviceInfo?: Record<string, string> };
    userHistory: { avgTransactionAmount: number; commonMerchants: string[]; homeCountry: string };
  }): Promise<PluginResult> {
    const start = Date.now();
    ...
    if (amountZScore > 3) { riskScore += 0.3; signals.push('UNUSUAL_AMOUNT'); }
    else if (amountZScore > 2) { riskScore += 0.15; signals.push('ELEVATED_AMOUNT'); }

    // New merchant
    if (transaction.merchantName && !userHistory.commonMerchants.includes(transaction.merchantName)) {
      riskScore += 0.1; signals.push('NEW_MERCHANT');
    }

    // High-risk amount thresholds
    if (transaction.amount > 10000) { riskScore += 0.2; signals.push('HIGH_VALUE'); }
    if (transaction.amount > 50000) { riskScore += 0.3; signals.push('VERY_HIGH_VALUE'); }

    // Round-number structuring pattern
    if (transaction.amount % 500 === 0 && transaction.amount >= 9000) { riskScore += 0.25; signals.push('STRUCTURING_PATTERN'); }
...
export class SpendingInsightsPlugin {
  manifest: PluginManifest = {
  ...
  async generateMonthlyInsight(userId: string, spendingData: {
    totalSpent: number; byCategory: Record<string, number>;
    vsLastMonth: number; budgetAdherence: number; topMerchants: string[];
  }): Promise<{ insights: string[]; recommendations: string[]; score: number }> {
    ...
    // Spending trend
    if (vsLastMonth > 0.15) insights.push(`Spending increased ${(vsLastMonth * 100).toFixed(0)}% vs last month.`);
    else if (vsLastMonth < -0.1) insights.push(`Great job! Spending decreased ${(Math.abs(vsLastMonth) * 100).toFixed(0)}% vs last month.`);

    // Category analysis
    const sortedCats = Object.entries(byCategory).sort((a, b) => b[1] - a[1]);
    if (sortedCats.length > 0) insights.push(`Highest spend category: ${sortedCats[0][0]} ($${sortedCats[0][1].toFixed(0)})`);

    // Dining recommendation
    const diningSpend = byCategory['FOOD_DINING'] ?? 0;
    if (diningSpend > totalSpent * 0.25) recommendations.push(`Dining is ${((diningSpend / totalSpent) * 100).toFixed(0)}% of spend. Consider meal prepping to save up to $${(diningSpend * 0.3).toFixed(0)}/month.`);

    // Budget adherence
    if (budgetAdherence < 0.8) recommendations.push('You exceeded your budget in several categories. Try the 50/30/20 rule.');
    else if (budgetAdherence >= 0.95) insights.push('Excellent budget adherence this month!');
...
export class AuditLoggerPlugin {
  manifest: PluginManifest = {
  ...
  async logEvent(eventName: string, payload: Record<string, unknown>, ctx: PluginContext): Promise<PluginResult> {
    const start = Date.now();
    const logEntry = {
  ...
  private maskPII(payload: Record<string, unknown>): Record<string, unknown> {
...
export class ReceiptScannerPlugin {
  manifest: PluginManifest = {
  ...
  async parseReceipt(imageBase64: string): Promise<{
    merchantName?: string; totalAmount?: number; currency?: string;
    date?: string; lineItems?: { description: string; amount: number }[];
    taxAmount?: number; category?: string; confidence: number;
  }> {
...
export class PluginRegistry {
  private plugins: Map<string, { manifest: PluginManifest; handler: object }> = new Map();
  private hooks: Map<string, Set<string>> = new Map(); // hook → plugin names

  register(manifest: PluginManifest, handler: object) {
    if (manifest.status === 'DEPRECATED') return;
  ...
  getPluginsForHook(hookName: string): string[] {
    const exact = this.hooks.get(hookName) ?? new Set<string>();
    const wildcard = this.hooks.get('*') ?? new Set<string>();
    return [...new Set([...exact, ...wildcard])];
  }

  async dispatchHook(hookName: string, payload: unknown, baseCtx: Omit<PluginContext, 'pluginName' | 'logger'>): Promise<Record<string, PluginResult>> {
...
pluginRegistry.register(new TransactionCategorizerPlugin().manifest, new TransactionCategorizerPlugin());
pluginRegistry.register(new FraudDetectorPlugin().manifest, new FraudDetectorPlugin());
pluginRegistry.register(new SpendingInsightsPlugin().manifest, new SpendingInsightsPlugin());
pluginRegistry.register(new AuditLoggerPlugin().manifest, new AuditLoggerPlugin());
pluginRegistry.register(new ReceiptScannerPlugin().manifest, new ReceiptScannerPlugin());
