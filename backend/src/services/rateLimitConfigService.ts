...
export type RateLimitTier = 'PUBLIC' | 'AUTHENTICATED' | 'PREMIUM' | 'ENTERPRISE' | 'ADMIN' | 'SERVICE';
export type CircuitState = 'CLOSED' | 'OPEN' | 'HALF_OPEN';
export type BackoffStrategy = 'EXPONENTIAL' | 'LINEAR' | 'FIBONACCI' | 'CONSTANT';

export interface RateLimitConfig {
  endpoint: string;
  method: string;
  limits: Record<RateLimitTier, { perMinute: number; perHour: number; perDay: number }>;
  burstAllowance?: number;       // extra % allowed for short bursts
  keyStrategy: 'USER_ID' | 'IP' | 'API_KEY' | 'SESSION_ID';
...
export interface RetryPolicy {
  ...
  baseDelayMs: number;
  maxDelayMs: number;
  jitter: number;       // 0-1: fractional jitter applied to delay
  retryOn: number[];    // HTTP status codes to retry
  noRetryOn: number[];  // Status codes never to retry
}
...
export interface CircuitBreakerConfig {
  ...
  timeoutMs: number;                 // Time to wait before HALF_OPEN
  halfOpenRequests: number;          // Max requests in HALF_OPEN
  monitoringWindowMs: number;        // Rolling window for failure counting
  fallbackBehavior: 'CACHE' | 'DEFAULT_RESPONSE' | 'ERROR' | 'QUEUE';
}

// ============================================================
// RATE LIMIT REGISTRY
// ============================================================
export const RATE_LIMIT_REGISTRY: RateLimitConfig[] = [
...
// ============================================================
export const CIRCUIT_BREAKERS: Record<string, CircuitBreakerConfig> = {
...
// ============================================================
// CACHE STRATEGY REGISTRY (Section 105)
// ============================================================
export interface CacheStrategy {
  key: string;
  ttlSeconds: number;
  strategy: 'WRITE_THROUGH' | 'WRITE_BEHIND' | 'REFRESH_AHEAD' | 'CACHE_ASIDE';
  invalidationEvents: string[];
  compressible: boolean;
  tier: 'L1_MEMORY' | 'L2_REDIS' | 'L3_CDN';
  maxSizeKB?: number;
}
...
export class RateLimitService {
   ...
   */
  computeRetryDelay(policy: RetryPolicy, attemptNumber: number): number {
   ...
   */
  shouldRetry(policy: RetryPolicy, statusCode: number, attemptNumber: number): boolean {
    if (attemptNumber >= policy.maxAttempts) return false;
    if (policy.noRetryOn.includes(statusCode)) return false;
   ...
   */
  getLimitForEndpoint(endpoint: string, tier: RateLimitTier): { perMinute: number; perHour: number; perDay: number } | null {
    const config = RATE_LIMIT_REGISTRY.find(r => endpoint.startsWith(r.endpoint));
  ...
  getCircuitBreakerStatus(service: string): {
    ...
    const config = CIRCUIT_BREAKERS[service];
   ...
   */
  buildCacheKey(dataType: string, params: Record<string, string>): string | null {
