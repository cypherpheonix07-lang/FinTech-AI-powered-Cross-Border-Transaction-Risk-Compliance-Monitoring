...
export type WormholeStability = 'STABLE' | 'FLUCTUATING' | 'COLLAPSING' | 'CRITICAL' | 'EVICTED';
export type CoordinateSystem = 'SCHWARZSCHILD' | 'KRUSKAL_SZEKERES' | 'EDDINGTON_FINKELSTEIN' | 'LEMAITRE';
export type ExoticMatterType = 'CASIMIR_VACUUM' | 'PHANTOM_ENERGY' | 'COSMIC_STRING' | 'NEGATIVE_MASS';

export interface WormholeHeader {
  sourceCoordinate: [number, number, number, number]; // [t, r, θ, φ]
  targetCoordinate: [number, number, number, number];
  metricTensorTrace: number;
  throatRadiusKm: number;
  exoticMatterRequiredKg: number;
  stabilityIndex: number;          // 0-1
  causalityGuardHash: string;      // Prevents closed timelike curves (CTC)
}

export interface WormholePacket {
  packetId: string;
  header: WormholeHeader;
  payloadHash: string;
  transitTimeElapsed: number;       // Proper time (should be near 0 for traversable)
  redundancyFactor: number;
  status: 'TRANSIT' | 'DELIVERED' | 'LOST_TO_SINGULARITY';
}

export interface SingularityGuard {
  guardId: string;
  energySurfaceDensity: number;
  tidalForceThreshold: number;
  hawkingRadiationLevel: number;
  eventHorizonDistortion: number;
  autoCloseTriggered: boolean;
}

export interface ER_EPR_Link {
  wormholeId: string;
  entangledPairId: string;         // Mapping ER bridge to EPR entanglement
  realityCoherence: number;
  crossRealitySettlementId?: string;
}
...
export class WormholeProtocolsService {
  private activeWormholes: Map<string, WormholeHeader> = new Map();
  private transitLogs: Map<string, WormholePacket> = new Map();
  private guards: Map<string, SingularityGuard> = new Map();
  ...
  private _initializeGuard(id: string): void {
    this.guards.set(id, {
      guardId: `guard-${id}`,
  ...
  injectPacket(bridgeId: string, payload: string): WormholePacket {
    const wh = this.activeWormholes.get(bridgeId);
    if (!wh) throw new Error('Wormhole bridge not active');
    const guard = this.guards.get(bridgeId);
    if (guard?.autoCloseTriggered) throw new Error('Wormhole guard triggered: Transfer blocked');

    const packet: WormholePacket = {
    ...
    if (wh.stabilityIndex < 0.7) {
      ...
      if (guard) guard.autoCloseTriggered = true;
    } else {
  ...
  getBridgeStatus(bridgeId: string): { stability: WormholeStability; exoticMatterReserves: number; tidalRisk: 'LOW' | 'MED' | 'HIGH' } {
    const wh = this.activeWormholes.get(bridgeId);
    if (!wh) return { stability: 'EVICTED', exoticMatterReserves: 0, tidalRisk: 'HIGH' };

    const status: WormholeStability = wh.stabilityIndex > 0.9 ? 'STABLE' : wh.stabilityIndex > 0.8 ? 'FLUCTUATING' : 'COLLAPSING';
    return {
      stability: status,
  ...
  mapToEntangledState(bridgeId: string, pairId: string): ER_EPR_Link {
    return {
      wormholeId: bridgeId,
  ...
  convertToKruskal(sCoord: [number, number, number, number]): [number, number, number, number] {
  ...
  getMetrics(): { openBridges: number; totalPackets: number; lossRate: number; avgStability: number } {
