...
export class BiasDetectionService {
   ...
   */
  async auditCognitiveBiases(userId: string, context: any = {}): Promise<CognitiveBiasReport[]> {
   ...
   */
  async shouldTriggerCoolOff(userId: string, amount: number): Promise<boolean> {
   ...
   */
  async computeWellnessScore(userId: string) {
   ...
   */
  async detectStressSpending(userId: string, transactionTimestamps: Date[]) {
