...
export type SwarmAlgorithm = 'ACO' | 'PSO' | 'BCO' | 'FIREFLY' | 'GREY_WOLF' | 'WHALE' | 'ARTIFICIAL_IMMUNE';
export type AgentRole = 'EXPLORER' | 'EXPLOITER' | 'SCOUT' | 'RECRUITER' | 'GUARDIAN' | 'ARBITRAGEUR';
export type MarketPhase = 'TRENDING_UP' | 'TRENDING_DOWN' | 'RANGING' | 'VOLATILE' | 'ILLIQUID' | 'CRISIS';
...
export interface SwarmAgent {
  ...
  position: number[];           // Current solution vector (e.g., portfolio weights)
  velocity?: number[];          // PSO velocity vector
  pBest?: number[];             // PSO personal best
  pBestFitness?: number;
  pheromoneTrail?: number[];    // ACO pheromone contributions
  fitness: number;              // Current fitness / utility score
  capital: number;              // Allocated capital USD
  riskBudget: number;           // Max drawdown allowed (0-1)
  memoryDepth: number;          // Steps of market memory
  learningRate: number;
  status: 'ACTIVE' | 'RESTING' | 'DEAD' | 'SUSPENDED';
...
export interface PheromoneMap {
  mapId: string;
  assetPairs: string[];         // e.g. ['BTC/USD', 'ETH/USD']
  pheromones: number[][];       // Matrix [assetA][assetB] = trail intensity
  evaporationRate: number;      // 0-1 per timestep
...
export interface SwarmConsensus {
  consensusId: string;
  topic: 'REBALANCE' | 'EXIT_POSITION' | 'HEDGE' | 'PAUSE_TRADING' | 'KILL_SWARM';
  proposedBy: string;
  votes: Record<string, boolean>; // agentId -> vote
  threshold: number;              // Fraction needed (e.g. 0.67)
  deadline: string;
...
export interface OrderRoutingDecision {
  ...
  side: 'BUY' | 'SELL';
  quantity: number;
  selectedVenues: Array<{ venue: string; quantity: number; estimatedFill: number; expectedSlippage: number }>;
  acoTrailScore: number;        // How strongly pheromones favor this route
  totalExpectedCost: number;    // Including spread, fees, market impact
...
export interface EmergentStrategy {
  ...
  type: string;                 // e.g. 'MOMENTUM_CLUSTER' | 'PAIR_ARBITRAGE' | 'TRIANGULAR_ARB'
  description: string;
  signals: Record<string, number>;
  backtestedSharpeRatio: number;
  backtestedMaxDrawdown: number;
  liveAdaptionsSoFar: number;
  status: 'CANDIDATE' | 'TESTING' | 'LIVE' | 'RETIRED';
}

export interface SwarmPortfolioAllocation {
  allocId: string;
  timestamp: string;
  assets: Record<string, number>;  // Asset -> weight (0-1, sum ≤ 1)
  psoIteration: number;
  globalBestFitness: number;
  expectedReturn: number;
  expectedVolatility: number;
  sharpeRatio: number;
  convergenceStatus: 'CONVERGING' | 'CONVERGED' | 'STAGNANT';
}

export interface MarketMicrostructureSim {
  simId: string;
  agents: SwarmAgent[];
  orderBook: { bids: Array<[number, number]>; asks: Array<[number, number]> };
  midPrice: number;
  spread: number;
  immediacy: number;            // Market liquidity immediacy
  resilience: number;           // Recovery speed after shock
  steps: number;
...
export class SwarmIntelligenceTradingService {
  private agents: Map<string, SwarmAgent> = new Map();
  private pheromoneMaps: Map<string, PheromoneMap> = new Map();
  private consensusVotes: Map<string, SwarmConsensus> = new Map();
  private emergentStrategies: Map<string, EmergentStrategy> = new Map();
  private gBest: { position: number[]; fitness: number } = { position: [], fitness: -Infinity };
  ...
  spawnAgent(params: {
    ...
    if (this.killSwitchActive) throw new Error('[SWARM] Kill switch active — no new agents permitted');
   ...
   */
  runPSOIteration(returns: Record<string, number[]>, covariances: Record<string, Record<string, number>>, iterations = 100): SwarmPortfolioAllocation {
    ...
    for (let iter = 0; iter < iterations; iter++) {
      for (const agent of agents) {
        ...
        if (fitness > (agent.pBestFitness ?? -Infinity)) {
          agent.pBest = [...weights]; agent.pBestFitness = fitness;
        }
        // Update global best
  ...
  private _computeSharpe(weights: number[], assets: string[], returns: Record<string, number[]>, _cov: Record<string, Record<string, number>>): number {
  ...
  private _normalize(arr: number[]): number[] {
  ...
  routeOrderACO(params: {
    ...
    venues: Array<{ name: string; liquidity: number; fees: number; slippage: number }>;
    ...
    for (let iter = 0; iter < 50; iter++) {
      for (const ant of ants) {
        ...
        if (selected >= 0) pheromones[selected] += 1 / (params.venues[selected].fees + 0.001);
      }
      // Evaporate
      for (let i = 0; i < pheromones.length; i++) pheromones[i] *= (1 - evapRate);
    }
   ...
   */
  bcoBidAskOptimization(midPrice: number, volatility: number, inventoryRisk: number): {
    bid: number; ask: number; spread: number;
    confidence: number; waggleDanceStrength: number;
  } {
    ...
    function waveDance(w: number) { return 1 / (1 + Math.exp(-w)); }
  }

  // ---- PHEROMONE MAP MANAGEMENT ----------------------------------------

  updatePheromones(mapId: string, signalAssets: string[], signalStrength: number): void {
  ...
  proposeConsensus(topic: SwarmConsensus['topic'], proposerId: string, thresholdFraction = 0.67): SwarmConsensus {
  ...
  castVote(consensusId: string, agentId: string, vote: boolean): { passed?: boolean; voteCount: number } {
    const c = this.consensusVotes.get(consensusId);
    if (!c || c.outcome) return { voteCount: 0 };
    ...
    if (quorum >= c.threshold) {
      ...
      if (c.outcome === 'PASSED' && c.topic === 'KILL_SWARM') this.activateKillSwitch();
    }
  ...
  activateKillSwitch(reason = 'CONSENSUS_VOTE'): void {
    this.killSwitchActive = true;
    for (const agent of this.agents.values()) agent.status = 'SUSPENDED';
    console.error(`[SWARM KILL SWITCH] Activated. Reason: ${reason}. All ${this.agents.size} agents suspended.`);
  }

  deactivateKillSwitch(authorizationToken: string): boolean {
    if (authorizationToken !== 'REGULATOR_OVERRIDE_TOKEN') return false;
    this.killSwitchActive = false;
    for (const agent of this.agents.values()) if (agent.status === 'SUSPENDED') agent.status = 'ACTIVE';
    return true;
  }

  // ---- EMERGENT STRATEGY DISCOVERY -------------------------------------

  scanForEmergentStrategies(marketData: Record<string, number[]>): EmergentStrategy[] {
    const discovered: EmergentStrategy[] = [];
    ...
    for (let i = 0; i < assets.length; i++) {
      for (let j = i + 1; j < assets.length; j++) {
        ...
        if (returns_i.length < 2) continue;
        const correlation = this._pearson(returns_i, returns_j);

        if (Math.abs(correlation) > 0.8) {
          const strat: EmergentStrategy = {
  ...
  private _pearson(a: number[], b: number[]): number {
  ...
  simulateMarketMicrostructure(agents: SwarmAgent[], steps: number, initialPrice: number): MarketMicrostructureSim {
    const sim: MarketMicrostructureSim = {
      simId: `sim-${Date.now()}`,
      agents,
      orderBook: { bids: [], asks: [] },
    ...
    for (let s = 0; s < steps; s++) {
      for (const agent of agents) {
        ...
        if (signal > 0.3) sim.orderBook.bids.push([sim.midPrice - sim.spread / 2, qty]);
        else if (signal < -0.3) sim.orderBook.asks.push([sim.midPrice + sim.spread / 2, qty]);
      }
  ...
  getSwarmStats(): { totalAgents: number; activeAgents: number; killSwitchActive: boolean; emergentStrategies: number; totalPnl: number; avgFitness: number } {
